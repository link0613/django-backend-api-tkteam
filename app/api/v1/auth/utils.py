...
def login_user(request, user):
    token, _ = Token.objects.get_or_create(owner=user)
...
class UserEmailFactoryBase(object):
    ...
    def from_request(cls, request, user=None, from_email=None, **context):
        ...
        return cls(
            ...
            domain=django_settings.AUTH.get('DOMAIN') or site.domain,
            site_name=django_settings.AUTH.get('SITE_NAME') or site.name,
...
class UserActivationEmailFactory(UserEmailFactoryBase):
    ...
    def get_context(self):
        ...
        context['url'] = django_settings.AUTH.get('ACTIVATION_URL').format(**context)
        return context
...
class UserPasswordResetEmailFactory(UserEmailFactoryBase):
    ...
    def get_context(self):
        ...
        context['url'] = django_settings.AUTH.get('PASSWORD_RESET_CONFIRM_URL').format(
            **context
        )
        return context
